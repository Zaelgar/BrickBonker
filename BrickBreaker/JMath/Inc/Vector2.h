...
using namespace JMath::Constants;
...
struct Vector2
{
	...
	{
		float x = JMath::Random::RandomFloatUniform(0.0f, 100.f);
		float y = JMath::Random::RandomFloatUniform(0.0f, 100.f);
...
static inline Vector2 CatmullRomSplineLerp(const std::vector<Vector2> points, const float t)
{
	...
	for (size_t i = 1; i < points.size() - 1; ++i)	// Loop through all points except beginning and end (which should remain 0)
	{
		pointVelocities[i] = (points[i + 1] - points[i - 1]) / 2.0f;
	}

	unsigned int splineT = static_cast<int>(floorf(t));
