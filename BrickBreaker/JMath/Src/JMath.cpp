...
void RandomPointsInRectangle(std::vector<JMath::Vector2>& points, JMath::Rect areaBounds, int numberOfPoints)
{
	...
	{
		// Why did I convert these floats to int? Do I need pixel perfection? Can't remember...
		//x = (rand() % (maxX - minX)) + minX;
		x = JMath::Random::RandomIntUniform(minX, maxX);
		//y = (rand() % (maxY - minY)) + minY;
		y = JMath::Random::RandomIntUniform(minY, maxY);

		points.emplace_back(Vector2(static_cast<float>(x), static_cast<float>(y)));
	}
}
